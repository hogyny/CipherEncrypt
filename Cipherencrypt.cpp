...
//function 4-1
	//-----------encrypted Caeser cipher and store to encrpted file as file.txt
	
	void encrypt(string textcontent){
		string result = "";
	int shift;
	cout << "Enter the required Encrypting key, only integer is allowed\n";
	cin >> shift;
	if (shift < 1 ){
		cout << "Only integer is required and must be greater than 1 for encryption process to start\n";
		cout << "This process is stop\n";
		
	}
	else{
		cout <<"Text encrption start------\n";
	    ...
	    cout <<"Encrption completed... Encrpted text below\n";
	    cout <<">>:  " << result <<"\n\n";
	}
	cout <<"Press any key on you keyboard to store encrpted data into a file\n";
	system ("pause");
	...
	if (input == 'y' || input == 'Y'){
		...
		ofstream myoutPut((name + ".txt").c_str());
	  	myoutPut << result;
	  	cout << "Congratulation you Encrypted file is stored...";
	}
	else{
	string name = "encyptedText.txt";
	ofstream myoutPut((name).c_str());
  	myoutPut << result;
  	cout << "Congratulation you Encrypted File is stored with default name as encyptedText.txt...";
	}
...
int main(){
	...
	cout << "This program is design to read text/ String and store the result in Encrpted mode\n";
